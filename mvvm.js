class Mvvm {
    constructor(options){
        ...
        this._data = data;
        //通过该函数把所有前台传来的data数据劫持
        Observe.observeData(data);
        //把所有的data数据代理到this(Mvvm对象)上
        this.mount(data);
        //解析模板数据
...
class Observe{
    ...
    mount(data,key,value,dep){
        // 将data中的数据一层层递归的通过defineProperty方式创建
        Object.defineProperty(data,key,{
            enumerable:true,
            get(){
                //Dep.target = watcher 存在的时候，添加到可观察对象数组中
            ...
            if(node.nodeType === 1){
                ...
                Array.from(nodeAttrs).forEach((attr)=>{
                    ...
                    if(name.startsWith('v-')){
                        node.value = vm[exp]; // 将a的值添加到input输入框内
                    }
                    ...
                    node.addEventListener('input',function (e) {
                        // 调用数据劫持中的set方法，从而触发 dep.notify()发布所有的订阅来更新界面数据
...
class Watcher{
    constructor(vm,exp,fn){
        ...
        let arr = exp.split('.');
        arr.forEach(function (k) {
            val = val[k]; // 该步骤会循环的调用改对象的get，所以就会把该watcher添加到观察数组中
        });
        Dep.target = null;
    }
